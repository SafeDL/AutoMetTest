"""
Training script for two_stream_cnn
"""
import os
...
import driving_data
from model import build_two_stream_cnn
...
for epoch in range(epochs):
    for step in range(int(driving_data.num_train_images / batch_size)):
        ...
        train_step.run(feed_dict={model.inputs[0]: xs_rgb, model.inputs[1]: xs_flow, y_true: ys})
        # Write logs at every iteration
        summary = train_loss_summary.eval(feed_dict={model.inputs[0]: xs_rgb, model.inputs[1]: xs_flow, y_true: ys})
    ...
    for step in range(int(driving_data.num_val_images / batch_size)):
        ...
        val_loss_value = loss.eval(feed_dict={model.inputs[0]: xs_rgb, model.inputs[1]: xs_flow, y_true: ys})
    ...
    if avg_val_loss < min_val_loss:
        ...
        checkpoint_path = os.path.join(LOGDIR, "udacity.ckpt")
