"""
Fine-tuning training script for two_stream_cnn
"""
import os
...
import driving_data
from model import build_two_stream_cnn, build_fine_tuned_two_stream_cnn

# Build fine-tuning model
pretrained_model = build_two_stream_cnn(weights_path="./save/carla_original.ckpt")
finetune_model = build_fine_tuned_two_stream_cnn(pretrained_model)
LOGDIR = './save'
...
# Define dynamic learning rate (learning rate decay strategy)
...
# Optimizer: Adam optimizer + dynamic learning rate
...
for epoch in range(epochs):
    for step in range(int(driving_data.num_train_images / batch_size)):
        xs_rgb, xs_flow, ys = driving_data.LoadTrainBatch(batch_size)
        train_step.run(feed_dict={finetune_model.inputs[0]: xs_rgb, finetune_model.inputs[1]: xs_flow, y_true: ys})
        # Write logs at every iteration
        summary = train_loss_summary.eval(feed_dict={finetune_model.inputs[0]: xs_rgb, finetune_model.inputs[1]: xs_flow, y_true: ys})
    ...
    for step in range(int(driving_data.num_val_images / batch_size)):
        xs_rgb, xs_flow, ys = driving_data.LoadValBatch(batch_size)
        val_loss_value = loss.eval(feed_dict={finetune_model.inputs[0]: xs_rgb, finetune_model.inputs[1]: xs_flow, y_true: ys})
